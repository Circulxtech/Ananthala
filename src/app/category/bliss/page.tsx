...
export default function BlissPage() {
  ...
  const handleAddMattressToCart = async () => {
    ...
    const mattressItem: CartItem = {
      id: `bliss-mattress-${mattressVariant}-${mattressDimension}-${selectedColor}-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`,
      name: "BLISS Mattress",
    ...
    if (pillowSize) {
      const pillowItem: CartItem = {
        id: `bliss-pillow-${pillowSize}-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`,
        name: "BLISS Pillow (Complimentary)",
    ...
    if (bedSpreadColor) {
      const bedSpreadItem: CartItem = {
        id: `bliss-bedspread-${bedSpreadColor}-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`,
        name: "BLISS Bed Spread (Complimentary)",
        image: "/luxury-plush-mattress-with-pillows-on-bed.jpg",
        size: bedSpreadColor,
        quantity: 1,
        price: 0,
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    {currentImages.map((image, index) => (
                      ...
                      >
                        <Image
                          src={image}
                          alt={`BLISS Mattress view ${index + 1}`}
                          fill
