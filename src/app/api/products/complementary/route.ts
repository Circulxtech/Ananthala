...
export async function GET(request: Request) {
  try {
    ...
    let query: any = { 
      status: "visible",
      productType: "complementary" // Only show complementary products
    }
    ...
    const products = await Product.find(query).select("_id productTitle imageUrls variants productType").limit(100)
    ...
    const productsWithPrice = products.map((product) => ({
      ...
      productType: product.productType || "complementary",
    }))
